...
"""
GeCCo (Gene Content Counter) is a Python tool that allows for the visualisation of normalised gene counts.
It was created for gene class predictions in the style of JGI, but also allows for custom files to be visualised.

Example:

    $ gecco [-h] -i Input_folder -o Output_folder -m {0,1,2,3,4}
...
class FastaReader:
    """
    This class reads the fasta files in the input folder and calculates the number of genes per species
        ...
        for file in os.listdir(self.input):
            if file.__contains__("IPR"):
                ...
                for key, value in tf_pfam_counts.items():
                    ...
                    self.tf_data["counts"].append(value/gene_count[str(file).split("_")[0]])
    ...
    def cazy_reader(self) -> pd.DataFrame:
        ...
        for file in os.listdir(self.input):
            if file.__contains__("CAZy"):
                cazy_families = {"AA": 0, "CBM": 0, "CE": 0, "GH": 0, "GT": 0, "PL": 0}
        ...
        for key, value in kegg_out.items():
            ...
            for key, value in kegg_out[species_name].items():
                if not key == "\\N":
                    ...
                    self.kegg_data["counts"].append(value/gene_count[species_name])
...
class KogData:
    ...
    def __init__(self, input_folder: str):
        ...
        self.column_order = ["Cell wall/membrane/envelope biogenesis ",
                             "Cell motility ",
...
class PepData:
    """
    Class for the visualisation of peptidase clans from IPR classification files. 
        ...
        for file in os.listdir(self.input):
            if file.__contains__("IPR"):
                ...
                for key, value in pep_clan_counts.items():
                    ...
                    self.pep_data["counts"].append(value/gene_count[str(file).split("_")[0]])
    ...
    def custom_reader(self) -> pd.DataFrame:
                    ...
                    for line in custom_file.readlines():
                        ...
                        if line.startswith("#"):
                            ...
                            if split_line[0] not in custom_dict:
                                ...
                                custom_dict[split_line[0]].append(split_line[1])
        ...
        for key, value in custom_out.items():
            ...
            for key, value in custom_out[species_name].items():
                ...
                self.custom_data["counts"].append(value/gene_count[species_name])
...
class HeatGenerator:
    """
    This class contains functions which allow the user to generate heatmaps from gene count data.
    The data, in the form of a pandas dataframe, will first be normalised by calculating the Z-score.

    Attributes:
    inputs (list): A list containing a pandas dataframe and the title of the y-axis [dataframe, title].
    output (str): The name of the folder where the figures should be saved.
    """
    def __init__(self, inputs: str, output: str):
        self.dataframe = inputs[0].fillna(0).select_dtypes(include='number').apply(stats.zscore).transpose()
        self.output = output
        self.header = inputs[1]

    def generate(self) -> px:
        """
        Takes the normalised dataframe and performs heirarchical clustering across the y-axis.
        Subsequently, a heatmap is generated for this data with the species along 
        the x-axis and the classes along the y-axis. The heatmap will be saved as an HTML file.
        """
        dendro_side = ff.create_dendrogram(self.dataframe, orientation='right')
        dendro_leaves = dendro_side['layout']['yaxis']['ticktext']
        dendro_leaves = list(map(int, dendro_leaves))

        factor_order = self.dataframe.loc[list(self.dataframe.index[dendro_leaves])]
        fig = px.imshow(
                    factor_order, 
                    labels = dict(
                        x="Species", 
